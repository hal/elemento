...
mkdir -p "$SITE_API_DOCS/$PACKAGE_DIR"
find "core/$SITE_API_DOCS/$PACKAGE_DIR" -type f -name "*.html" -maxdepth 1 \
    -exec cp -v {} "$SITE_API_DOCS/$PACKAGE_DIR" \;
for module in ${MODULES[@]}; do
    mkdir -p "$SITE_API_DOCS/$PACKAGE_DIR/$module"
    find "$module/$SITE_API_DOCS/$PACKAGE_DIR/$module" -type f -name "*.html" -maxdepth 1 \
        -exec cp -v {} "$SITE_API_DOCS/$PACKAGE_DIR/$module" \;
done
