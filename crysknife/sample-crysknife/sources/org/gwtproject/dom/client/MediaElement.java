 ...
 *
 * <p>See <a href="http://www.w3.org/TR/html5/video.html">W3C HTML5 Video and Audio</a>
 */
@JsType(isNative = true, name = "Object", namespace = JsPackage.GLOBAL)
public class MediaElement extends Element {

  /** Constant returned from {@link #canPlayType(String)}. */
  @JsOverlay public static final String CAN_PLAY_PROBABLY = "probably";

  /** Constant returned from {@link #canPlayType(String)}. */
  @JsOverlay public static final String CAN_PLAY_MAYBE = "maybe";
  ...
  @JsOverlay public static final String PRELOAD_AUTO = "auto";

  /** Constant used by {@link #getPreload()} and {@link #setPreload(String)}. */
  @JsOverlay public static final String PRELOAD_METADATA = "metadata";

  /** Constant used by {@link #getPreload()} and {@link #setPreload(String)}. */
  @JsOverlay public static final String PRELOAD_NONE = "none";

  protected MediaElement() {}

  /**
   * Returns {@code true} if the native player is capable of playing content of the given MIME type.
   *
   * @param type a String representing a MIME type
   * @return one of {@link #CAN_PLAY_PROBABLY}, {@link #CAN_PLAY_MAYBE}, or {@link #CANNOT_PLAY}
   */
  @JsOverlay
  ...
  /**
   * Returns a {@link TimeRanges} object indicating which portions of the source have been buffered
   * locally.
   *
  ...
  /**
   * Returns the default playback rate, where 1.0 corresponds to normal playback. If no rate has
   * been set, 1.0 is returned.
   *
  ...
  /**
   * Returns the duration of the source media stream, in seconds. If the duration is unknown, {@link
   * Double#NaN} is returned. For unbounded media streams, {@link Double#POSITIVE_INFINITY} is
   * returned.
   *
  ...
  /**
   * Returns the type of error that has occurred while attempting to load and play the media. If no
   * error has occurred, {@code null} is returned.
   *
  ...
  @JsProperty private MediaError error;

  /**
   * Returns the time to which the media stream was seeked at the time it was loaded, in seconds, or
   * 0.0 if the position is unknown.
   *
  ...
  /**
   * Returns the network state, one of {@link #NETWORK_EMPTY}, {@link #NETWORK_IDLE}, {@link
   * #NETWORK_LOADING}, or {@link #NETWORK_NO_SOURCE}.
   *
  ...
  /**
   * Returns the playback rate, where 1.0 corresponds to normal playback. If the rate has not been
   * set, 1.0 is returned.
   *
  ...
  @JsProperty private Object playbackRate;

  /**
   * Returns a {@link TimeRanges} object indicating which portions of the source have been played.
   *
   * @return a {@link TimeRanges} instance, or {@code null}.
   */
  @JsProperty
  ...
  /**
   * Returns the preload setting, one of {@link #PRELOAD_AUTO}, {@link #PRELOAD_METADATA}, or {@link
   * #PRELOAD_NONE}.
   *
  ...
  /**
   * Returns the current state of the media with respect to rendering the current playback position,
   * as one of the constants {@link #HAVE_CURRENT_DATA}, {@link #HAVE_ENOUGH_DATA}, {@link
   * #HAVE_FUTURE_DATA}, {@link #HAVE_METADATA}, or {@link #HAVE_NOTHING} .
   *
  ...
  /**
   * Returns a {@link TimeRanges} object indicating which portions of the source are seekable.
   *
   * @return a {@link TimeRanges} instance, or {@code null}.
   */
  @JsProperty
  ...
  /**
   * Returns the time corresponding to the zero time in the media timeline, measured in seconds
   * since midnight, January 1 1970 UTC, or {@link Double#NaN} if none is specified.
   *
  ...
  /**
   * Returns {@code true} if the media player should display interactive controls (for example, to
   * control play/pause, seek position, and volume), {@code false} otherwise.
   *
  ...
  /**
   * Returns {@code true} if playback has reached the end of the media, {@code false} otherwise.
   *
  ...
  /**
   * Returns {@code true} if autoplay is enabled, {@code false} otherwise. When autoplay is enabled,
   * the user agent will begin playback automatically as soon as it can do so without stopping.
   *
  ...
  /**
   * Returns {@code true} if the user agent is to seek back to the start of the media once playing
   * has ended, {@code false} otherwise.
   *
  ...
  /**
   * Returns {@code true} if the volume is to be muted (overriding the normal volume setting),
  ...
  /**
   * Returns {@code true} if the playback position is in the process of changing discontinuously,
   * e.g., by use of the interactive controls, {@code false} otherwise.
   *
  ...
  /**
   * Changes the preload setting to one of {@link #PRELOAD_AUTO}, {@link #PRELOAD_METADATA}, or
   * {@link #PRELOAD_NONE}.
   *
