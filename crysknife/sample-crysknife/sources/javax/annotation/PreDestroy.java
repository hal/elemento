 ...
 * instance is in the process of being removed by the container. The method annotated with
 * PreDestroy is typically used to release resources that it has been holding. This annotation MUST
 * be supported by all container managed objects that support PostConstruct except the application
 * client container in Java EE 5. The method on which the PreDestroy annotation is applied MUST
 ...
 * Exception <i>Note: A PreDestroy interceptor method must not throw application exceptions, but it
 ...
 * a PreDestroy interceptor method returns a value, it is ignored by the container.</i></li>
 ...
 * <li>If the method throws an unchecked exception it is ignored except in the case of EJBs where
 * the EJB can handle exceptions.</li>
 * </ul>
 * 
