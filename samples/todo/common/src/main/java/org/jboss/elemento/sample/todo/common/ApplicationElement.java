    ...
    public ApplicationElement(TodoRepository repository) {
        ...
        this.root = section().css("todoapp")
                ...
                .add(main = section().css("main")
                        ...
                        .add(label().apply(l -> l.htmlFor = "toggle-all").text("Mark all as complete"))
                        .add(list = ul().css("todo-list"))
                        .element())
                .add(footer = footer().css("footer")
                        .add(count = span().css("todo-count").html(Messages.items(0)).element())
                        .add(ul().css("filters")
                                .add(li().add(filterAll = a(Filter.ALL.fragment()).text("All").element()))
                                .add(li().add(filterActive = a(ACTIVE.fragment()).text("Active").element()))
                                .add(li().add(
                                        filterCompleted = a(COMPLETED.fragment()).text("Completed").element())))
