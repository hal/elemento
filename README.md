...
Elemento is ready to be used with J2CL and GWT 3. It has no dependencies to `gwt-user`. There are methods that use safe HTML classes. But these classes are taken from [`org.gwtproject.safehtml:safehtml`](https://github.com/Vertispan/gwt-safehtml/). In case you still need to use the safe HTML classes from `gwt-user`, you can switch to another versions `GWT2-SNAPSHOT` resp. `x.y.z-gwt2`.
